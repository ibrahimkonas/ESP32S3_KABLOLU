...
class _AnaPanelState extends State<AnaPanel> {
  ...
  Future<void> _saveCustomHost(String value) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(clean.isEmpty ? 'Manuel host temizlendi' : 'Manuel host kaydedildi: $clean')),
      );
    }
    if (clean.isEmpty) {
  ...
  Future<void> _exportReports() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('CSV kaydedildi: ${file.path} (panoya kopyalandı)')),
      );
      if (!mounted) return;
      showDialog(
        context: context,
        ...
        Padding(
          ...
          child: ElevatedButton(
            onPressed: () {
              ...
              if (host.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Önce geçerli bir host/IP girin')),
                );
                return;
              }
    ...
    return Card(
      child: ListTile(
        ...
        trailing: IconButton(
          ...
          onPressed: (_isResolvingMdns || usingManual) ? null : _resolveMdnsHost,
        ),
      ),
    );
  }
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Manuel IP/Host', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            TextField(
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Uzaktan kontrol', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 6),
            Row(
              children: [
                _chip(isOto ? 'AUTO' : 'MANUAL', isOto ? Colors.blue : Colors.orange),
                const SizedBox(width: 8),
                _chip('Start/Stop kilitli', Colors.grey),
              ],
            ),
            const SizedBox(height: 6),
            const Text('Start/Stop/Mod komutları sahadan verilmeli.'),
          ],
        ),
      ),
    );
